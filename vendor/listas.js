 let cont_table = new Object();
 
cont_table.CargadorContenidos = function(objeto_json,encabezados,exploradorTablas,NameDivList,NameDivIndex,wTabla,num_inicio,num_intervalos,elimina_id,dirEliminar,edit_id,dirEditar,funcion,dirlink,funcion2,funcion3,orden,title) {	
this.wTabla = wTabla;//id de la tabla cabecera del listado para obtener el ancho de cada celda
this.num_inicio = num_inicio;//hace referencia a si la tabla tiene o no dos ../images de eliminado y editado su valor es 0 o 2
...
cont_table.CargadorContenidos.prototype = {
...
{ texto='<span  id="'+this.exploradorTablas+'atras"  ><<< </span>'
	...
	this.atras(help,this.top,this.exploradorTablas);
	return help;
	},
...
crea_indice : function (n_table)	
{
	...
	$("#"+this.NameDivIndex).append(texto)
		...
		$(help).css("cursor","pointer")
		this.asignar(help,this.top,this.exploradorTablas);
	},
...
{texto='<span  id="'+this.exploradorTablas+'adelante"> >>> </span>'
	...
	$(help).css("cursor","pointer")
		this.adelante(help,this.top,this.exploradorTablas);	
		return help;
	},
 ...
 crea_tabla : function(n_table)//listo
{
		...
		tabla.setAttribute('class','table listadoGeneral');
...
var id_tr;
 ...
 var id_td_1;
	...
	switch(this.num_inicio){
		...
		if(this.eliminaId){
			if(this.orden){
			id_tr=this.NameDivList+'_'+datoOrden+'_tr_'+nFila;}
			else{id_tr=this.NameDivList+'_  tr_'+nFila;}
		...
		case  2:
                    id_td_0=this.NameDivList+'_td_0_'+nFila;//se crea el id de editar
                    id_td_1=this.NameDivList+'_td_1_'+nFila;//se crea el id de eliminar
                    ...
                    elmTD.innerHTML='<img src="../images/b_edit.png" id="'+id_td_0+'" align="top" width="20" class="imagen1" />&nbsp;&nbsp;<img class="imagen2"  src="../images/b_drop.png" id="'+id_td_1+'" align="top" width="20" />';
...
//regresar  procesarlinea
		...
		case  1:
		id_td_0=this.NameDivList+'_td_0_'+nFila;//se crea el id de eliminar
		elmTD = this.elmTR.insertCell(0);
		var tabla2=document.getElementById(this.wTabla);
		var tr=tabla2.rows[0];
		////alert (tabla2.id);
		...
		if(!this.dirlink)
		{ elmTD.setAttribute('nowrap','nowrap');
                    elmTD.innerHTML='<img src="../images/b_drop.png" width="20" height="16" />';
		if (navigator.appName == "Netscape"){ 
				 elmTD.setAttribute('style','cursor:pointer'); 
				} 
				else if (navigator.appName.indexOf("Explorer") != -1) { 
		...
		loaderLink=this;
		loaderLink.procesarlinea.call(loaderLink,help_2,dato,loaderLink.onload,DatosProcesar);}
		else{elmTD.setAttribute('nowrap','nowrap');
			elmTD.innerHTML='<a href="'+this.dirlink+'&dato='+dato+'" target="_blank"><img src="../images/b_drop.png"  width="20" height="16" /></a>';
		...
		id_td_0=this.NameDivList+'_td_0_'+nFila;//se crea el id de eliminar
		id_td_1=this.NameDivList+'_td_1_'+nFila;//se crea el id de eliminar
		var id_td_2=this.NameDivList+'_td_2_'+nFila;//se crea el id de eliminar
		
		elmTD = this.elmTR.insertCell(0);
		elmTD.setAttribute('width','13');
                elmTD.setAttribute('nowrap','nowrap');
		elmTD.innerHTML='<img id="'+id_td_0+'" src="../images/entrada.png" align="top" width="20" height="20" style="cursor:pointer"title="Eliminar Cita" /><img id="'+id_td_1+'"  align="top"  width="20" height="20" src="../images/ui-icons_lapiz.png"align="top" style="cursor:pointer"title="Edicion de Citas "/><img id="'+id_td_2+'" src="../images/ui-icons_nota.png" align="top"  width="20" height="20" src="../images/entrada.png" align="top" style="cursor:pointer"title="Atender a Paciente"/>';
		...
		elmTD.setAttribute('width',td_w);
		elmTD.setAttribute('id',id_td_0);
		elmTD.setAttribute('align','center');if (navigator.appName == "Netscape"){ 
				 elmTD.setAttribute('style','cursor:pointer'); 
				} 
				else if (navigator.appName.indexOf("Explorer") != -1) { 
				 elmTD.style.setAttribute('cursor','pointer'); }
                elmTD.innerHTML='<img src="../images/b_edit.png" width="20" height="16" />';
		help_2 ="td[id='"+id_td_0+"']";		 
                if(!this.onload)
                        {this.procesar_linea(help_2,dato,this.eliminaId,this.dirEliminar);}
                else{
		...
		var numInicio=this.num_inicio-3;
		if(this.title){
			id_tr='tr_'+nFila;
			this.elmTR.setAttribute('id',id_tr);
			help_2 ="tr[id='"+id_tr+"']";
...
{	  
		...
		var DatosProcesar=this.objeto_json.datos[nFila]
                if (this.objeto_json.nombreCabecera != '')
                    DatosProcesar['nombreCabecera'] = this.objeto_json.nombreCabecera ;
		var tabla2=document.getElementById(this.wTabla);
		var tr=tabla2.rows[0];	
		////alert(dato)
		...
		for (var i=0;i<NumeroCeldas;i++){
			...
			elmText = document.createTextNode(dato);
                        if (tr.cells[numInicio].style.display != ""){
                            elmTD.setAttribute('style',"display:"+tr.cells[numInicio].style.display);
                        }
			...
			elmTD.appendChild(elmText);
                        ...
                        inputHidden.setAttribute('id',cabDato);
                        inputHidden.setAttribute('value',dato);
			elmTD.appendChild(inputHidden);
...
adelante : function (help,top,exploradorTablas)
...
atras:function(help,top,exploradorTablas)

{$(help).click(function (){
        //alert(exploradorTablas);
	var tope=parseInt(document.getElementById(exploradorTablas).value);
	if (1<tope)
	{tope=tope-1;
	var i="lb_"+exploradorTablas+tope;
...
asignar:function(help,top,exploradorTablas){//donde help es a la cual se le va a aplicar la funcion click e (i) es el id de quien llama la funcion
 $(help).click(function (){
