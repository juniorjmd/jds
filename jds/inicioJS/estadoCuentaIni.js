...
buscarCartera($("#IdCuentaHD").val(),llenaCartera);
...
var fecha =mes+"/"+dia+"/"+anio;
	$("#fecha").html(fecha) ;
	var datos_json = "tabla=" + encodeURIComponent('abonoscartera')+
...
$("#valorAbono").keydown(function(tecla){
	...
	if(r==13){
			...
			}else{alert("el valor debe ser un numero ");}
			}
	});	
...
$("#guardarAbono").click(function(){ 
		 ...
		 let num1=parseInt($("#valorAbono").val())
                 let numero =  $("#deudaTotal").html()
                        .split("$").join('')
                        .split(".").join('')
                        .split(",").join('') ;
                 //alert(numero);
		 let num2=parseInt(numero);
		
		if(num2<num1){alert("el valor que esta insertado esta por ensima de la deuda corrija por favor")
				$("#valorAbono").focus()
			}else{
				if(num1!=0){
				fecha=$("#fecha").html()
    		...
    		cliente=$("#idClienteAbono").html() 	
				...
				"respuesta="+ encodeURIComponent('nuevoAbono')+"&nocache=" + Math.random();
			carga_insert(datos_json,function (){var respuesta_json = this.req.responseText;
			console.log(respuesta_json)
		abonosCartera();$("#nuevoAbono").css('display', 'none');
                
                            $("#TotalActual").html(formatNumber.new( num2-num1, "$") );
		});	}else{alert("el valor no puede ser cero '0'");	$("#valorAbono").focus();}
			}
 ...
 num2=parseInt($("#deudaTotal").html()
		...
		fecha=$("#fechaliquidar").html()
    		...
    		cliente=$("#idClienteAbonoliquidar").html() 	
				...
				"respuesta="+ encodeURIComponent('nuevoAbono')+"&nocache=" + Math.random();
				carga_insert(datos_json,function (){var respuesta_json = this.req.responseText;
		abonosCartera();$("#liquidar").css('display', 'none');
                $("#TotalActual").html('$0');
		});	}else{$("#liquidar").css('display', 'none');alert("esta deuda ya fue pagada")}
	});	
...
var cargadorLista= new T_find.CargadorContenidos(objeto_json,encabezados,'normalNewAbono','tablasListaAbono','indiceListaAbono','listarTablaAbono',0,15,false,false,false,false,false,false);
	...
	var texto="No se encuentra registrado ningun Abono en la base de datos";
...
var objeto_json = eval("("+respuesta_json+")");
var num_filas = objeto_json.filas;
log(JSON.stringify(objeto_json))
if (num_filas > 0)
{ $("#idCliente").html(objeto_json.datos[0]['idCliente']);
  ...
  $("#valorInicial").html(formatNumber.new(  objeto_json.datos[0]['valorInicial'], "$"));
   $("#abonoInicial" ).html(formatNumber.new(  objeto_json.datos[0]['abonoInicial'], "$"));
    $("#numCuotas").html(objeto_json.datos[0]['numCuotas']);
  $("#intervalo").html(objeto_json.datos[0]['intervalo']);
      $("#valorCuota").html(formatNumber.new( objeto_json.datos[0]['valorCuota'], "$"));
     $("#TotalActual").html(formatNumber.new( objeto_json.datos[0]['TotalActual'], "$"));
     $("#TotalInicial").html(formatNumber.new( objeto_json.datos[0]['TotalInicial'], "$"));
    $("#atras").attr('href',"listadoCreditosProveedor.php?tabla1=clientes&idCliente="+objeto_json.datos[0]['idCliente']+"&tabla2=cartera&dir=estadoCuenta.php");
	}}

function abonosCartera(){
 
inicioListar('abonoscartera',list_res_abonos,null,null,'idFactura',true,$("#IdCuentaHD").val(),null,null,'Abono')
