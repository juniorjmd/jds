...
$("input[id='busquedaPacienteRegistrado']").keyup(function (){
	var dato=$(this).val();
	if (Trim(dato)!=""){
	var query="select * from proveedores where `idCliente`= ";
	...
	"&dato=" + encodeURIComponent(dato)+		
		...
		inicioListar('proveedores',function(){
					var respuesta_json = this.req.responseText;	
					var objeto_json = eval('('+respuesta_json+')');	
					var num_filas = objeto_json.filas;var texto;
					if (num_filas>0){
						texto="";
						$("#razonSociallb").html(objeto_json.datos[0]['razonSocial']);}else{texto="la cedula no registrada"; $("#razonSociallb").html(""); 		
						}$("#errorProveedor").html(texto);},null,null,'idCliente',1,dato,'nit',null,'sL')																																																																																						   }else{var texto=" ";
		$("#razonSociallb").html(texto);
			$("#errorProveedor").html(texto);
			$(this).focus();}
...
$('#buscaProveedor').click(function(){
    llenaBusqueda()
    $.colorbox({overlayClose:false, inline:true, href:"#busquedaProveedor",width:"90%", height:"40%"});
								});
