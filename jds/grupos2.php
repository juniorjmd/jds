...
<?php
 ...
 $conn= cargarBD();
  $formDir="grupos2.php";
 if($_GET['llamado']){
	 $formDir="grupos2.php?llamado=".$_GET['llamado'];
...
$stmt = $conn->stmt_init();
$query="INSERT INTO `grupo2` VALUES (NULL, UCASE('".$_POST['nombreGrupo']."'));";
//echo $query;
$stmt->prepare($query);
if(!$stmt->execute()){
...
<?php 
$query2="SELECT * 
FROM  `grupo2` ORDER BY   `idGrupo` ASC ";
//echo $query2;
...
$("#Nuevo").click(function(e){
	...
	dato='dato='+encodeURIComponent($(this).attr('id'))
		  +"&tabla="+encodeURIComponent("grupo2")
