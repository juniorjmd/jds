...
$(document).keydown(function(e){
	...
	$('#btn_revisar_datos').click(function(){
				var cont = 0;
				var cantInicial = 0;
				var cantInicial_old = 0;
				var datos = "";
		...
		$('.lineas').each(function(){
			datos = '';
				...
				cantInicial_old = parseInt($(this).find('#cantInicial').val())
				cantInicial = parseInt($(this).find('#nw_cantInicial').val())
				if (cantInicial_old != cantInicial ) {
					datos = "<input type='hidden' value='"+cantInicial+"' name='array_poductos["+ cont +"][cantidad]'/>";
					datos += "<input type='hidden' value='"+idProducto+"' name='array_poductos["+ cont +"][id_producto]' />";	
					$('#contenedor_datos').append(datos);
					cont++;
				}
			...
			if (cont > 0){
				 $('#btn_envio_datos').trigger('click')
			}else{alert('no se ha realizado ningun cambio en los productos existentes!!!')} 
...
<form action="../inventario/" method="post">
  <input type="submit" value="Submit" style='display:none' id='btn_envio_datos'>
  <div id='contenedor_datos'>
  </div>
</form>
 <input type="button" id='btn_revisar_datos' value="Enviar"  class='flotante'>
...
while ($row = $result->fetch_assoc()) {
	...
	<td><input type='text' value='{$row['cantInicial']}' name='array_poductos[{$cont}][cantidad]' id='nw_cantInicial' class='nuevo_campo' onkeypress='return valida(event)'/>
		<input type='hidden' value='{$row['idProducto']}' name='array_poductos[{$cont}][id_producto]' id='id_producto' />
	</td>
