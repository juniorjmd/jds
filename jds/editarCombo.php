...
$cancel="Cancelar...";
if($_POST['nombre']){	
	$query="UPDATE  `combos` SET `nombre` = '".$_POST['nombre']."',
	`precioVenta`='".$_POST['Pventa']."',
	`imagen`    =  '".$_POST['imagenGuardar']."'	
	 WHERE CONVERT(  `combos`.`idCombo` USING utf8 ) =  '".$_POST['codigo']."' LIMIT 1 ;";
	$stmt = $conn->stmt_init();
	$stmt->prepare($query);
	$cancel="Regresar...";
...
$query2="SELECT * FROM `combos` WHERE `idCombo`= '".$_POST['codigo']."'";
//echo $query2;
...
$codigo=substr($_POST['codigo'],2);

$query2="SELECT * FROM `relacioncombo` WHERE `id`= '".$_POST['codigo']."'";
//echo $query2;
$result = $conn->query($query2);
$filaInsertados="";
$ComboImagen="imagenes/Pregunta.png";
...
<table width="950" height="362" border="0" >
  ...
  <tr>
    ...
    <td colspan="2"><input type="text" name="nombre" id="nombre" class="articulo" value="<?php echo $nombre;?>" ></td>
    </tr>
    ...
    <td width="240"><input type="number" name="Pventa" id="Pventa" class="articulo" size="" value="<?php echo $precioVenta;?>"></td>
    ...
    <td width="273" height="87"  align="right" valign="top">
        <p><input type="submit" id="nuevoProducto" value="+ Producto" class="boton" style=""> <input type="submit" id="enviar" value="Registrar" class="boton" style=""> 
            <input type="button" id="cancelar" value="<?php echo $cancel;?>" class="boton">
          </p>
        </blockquote>
      </blockquote>
    </blockquote></td>
