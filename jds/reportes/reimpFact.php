...
if (isset($num_factura)){
	if (trim($num_factura)!='' ){
		$query="SELECT j.idVenta as idVenta, cantidadVendida , valorParcial , valorIVA, valorTotal, fecha, hora, usuario,nit,razonSocial, tipoDeVenta ,estadoFactura , U.nombre , U.apellido
				FROM  `ventas` j
				left JOIN ventacliente y ON j.idVenta = y.`idVenta` 
				left join clientes F on y.idCliente = F.idCliente 
		...
		if ($datosNum > 0){
			...
			$activo = 'disabled';
			if ($estadoFactura != 'A'){$estFact='Cancelada';}else{$estFact='Activa';}
		?>
		<style>
       ...
       <input type="hidden" name="iva" id="iva" value="<?php echo $iva; ?>"/>
		...
		<button type="button" class="btn btn-default" id='btn_visual'>
		<span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>_visualizar</button> 
		<button type="button" class="btn btn-default">
		<span class="glyphicon glyphicon-print" aria-hidden="true"></span>_imprimir</button>  </div>
		...
		<input type='submit' style='display:none' id='enviar1'>
		</form>
...
<form autocomplete="off" method="post" target=''>
	<input type="number" id="prt_num_factura" name="prt_num_factura" value="<?php echo $num_factura; ?>"  style='display:none' >  
	<input type='submit' style='display:none' id='enviar2'>
</form>
</body>
