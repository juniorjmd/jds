...
<div class="panel panel-default">
  ...
  <!-- Default panel contents -->
		...
		if ($datosNum > 0){
			...
			while ($row = $result->fetch_assoc()) {
			   ...
			   echo  '<tr><td>'.$row['idProducto'].'</td>';   
				...
				echo '<td  align="right">'.$row['valorTotal'].'</td>';
				/*
				echo '<td align="center"><select id="select_'.$row['idLinea'].'" name="select_'.$row['idLinea'].'">';
				for($i=0; $i<=$row['cantidadVendida']; $i++)
        ...
        <a class="btn btn-primary btn-lg" href="../devoluciones">
 <span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> regresar </a>
 <a class="btn btn-primary btn-lg" id='devolver_todo'>
...
?>
 <form action="devolucion_final.php" method="post" autocomplete='off'>
		...
		<input type='submit' id='segundo_form'>
</form>
